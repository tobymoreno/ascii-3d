...
fn main() {
    unsafe {
        raylib::ffi::SetConfigFlags(ConfigFlags::FLAG_WINDOW_TRANSPARENT as u32);
    }

    let (mut rl, thread) = raylib::init()
        .size(WIDTH, HEIGHT)
        .title("ascii-3d raylib transparent-only proof")
        .build();
...
def main() -> None:
    ...
    print("Applied raylib transparent-only proof patch.")
