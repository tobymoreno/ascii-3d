...
def patch_cargo() -> None:
    ...
    if 'raylib = ' not in text:
        text = text.rstrip() + '\nraylib = "6.0.0"\n'

    path.write_text(text)

def park_cocoa_overlay() -> None:
    path = Path("src/graphics/macos_overlay.rs")
    if not path.exists():
        path.parent.mkdir(parents=True, exist_ok=True)

    path.write_text(r'''use std::error::Error;
...
def write_raylib_demo() -> None:
    path = Path("src/bin/raylib_overlay_demo.rs")
    path.parent.mkdir(parents=True, exist_ok=True)

    path.write_text(r'''use std::time::{Duration, Instant};
...
fn draw_demo_frame(d: &mut RaylibDrawHandle<'_>, elapsed: f32) {
    ...
    d.draw_text(
        "raylib transparent primitive overlay",
        42,
        HEIGHT - 44,
        20,
        Color::GREEN,
...
def main() -> None:
    ...
    park_cocoa_overlay()
    write_raylib_demo()
    print("Applied raylib transparent overlay demo patch.")
