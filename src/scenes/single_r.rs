...
const SCENE_ASSET: &str = "assets/scenes/single_r_axes.scene.json";

#[derive(Debug, Clone, Deserialize)]
struct SingleRSceneAsset {
    name: String,
    version: u32,
...
fn load_scene_assets() -> io::Result<(
    ...
    let scene: SingleRSceneAsset = read_json(SCENE_ASSET)?;

    if scene.version != 1 {
        return Err(io::Error::other(format!(
            "unsupported single_r scene version {}",
            scene.version,
        )));
    }
    ...
    let axes_node = scene
        ...
        .ok_or_else(|| io::Error::other("single_r scene is missing cartesian_axes node"))?
        .clone();
    ...
    let word_node = scene
        ...
        .ok_or_else(|| io::Error::other("single_r scene is missing bezier_word node"))?
        .clone();

    if word_node.parent.as_deref() != Some(&axes_node.id) {
        return Err(io::Error::other(
            "single_r word node must be parented to the Cartesian axes node",
        ));
    }
    ...
    let word_metadata: WordMetadata = read_json("assets/words/single_r.metadata.json")?;
...
pub fn render(canvas: &mut Canvas, stroke_character: Option<char>) -> io::Result<()> {
    ...
    canvas.draw_text(Point2::new(2, 1), "Scene: single_r word parent");
    canvas.draw_text(Point2::new(2, 2), &format!("Asset: {}", scene.name));
    canvas.draw_text(
        Point2::new(2, 24),
        "Hierarchy: axes_root -> word_single_r -> glyph_R_0",
    );
    canvas.draw_text(
        Point2::new(2, 25),
        "Glyph R uses proportional 0..1 local space",
    );
...
#[cfg(test)]
mod tests {
    use super::{SingleRSceneAsset, read_json};

    #[test]
    fn single_r_scene_asset_loads() {
        let scene: SingleRSceneAsset =
            read_json("assets/scenes/single_r_axes.scene.json").expect("scene should load");
