...
const SCENE_ASSET: &str = "assets/scenes/crew_axes.scene.json";

#[derive(Debug, Clone, Deserialize)]
struct CrewSceneAsset {
    name: String,
    version: u32,
...
fn load_scene_assets() -> io::Result<(
    ...
    let scene: CrewSceneAsset = read_json(SCENE_ASSET)?;

    if scene.version != 1 {
        return Err(io::Error::other(format!(
            "unsupported crew scene version {}",
            scene.version,
        )));
    }
    ...
    let axes_node = scene
        ...
        .ok_or_else(|| io::Error::other("crew scene is missing cartesian_axes node"))?
        .clone();
    ...
    let word_node = scene
        ...
        .ok_or_else(|| io::Error::other("crew scene is missing bezier_word node"))?
        .clone();

    if word_node.parent.as_deref() != Some(&axes_node.id) {
        return Err(io::Error::other(
            "crew word node must be parented to the Cartesian axes node",
        ));
    }
    ...
    let word_metadata: WordMetadata = read_json("assets/words/crew.metadata.json")?;
...
pub fn render(canvas: &mut Canvas, stroke_character: Option<char>) -> io::Result<()> {
    ...
    canvas.draw_text(Point2::new(2, 1), "Scene: crew word parent");
    canvas.draw_text(Point2::new(2, 2), &format!("Asset: {}", scene.name));
    canvas.draw_text(
        Point2::new(2, 24),
        "Hierarchy: axes_root -> word_crew -> glyph_C_0 -> glyph_R_1 -> glyph_E_2 -> glyph_W_3",
    );
    canvas.draw_text(
        Point2::new(2, 25),
        "CREW uses four monospace 0..1 glyph boxes",
    );
    canvas.draw_text(
        Point2::new(2, 26),
        "Runtime stroke character cycling applies to all CREW glyphs",
    );
...
#[cfg(test)]
mod tests {
    use super::{CrewSceneAsset, read_json};

    #[test]
    fn crew_scene_asset_loads() {
        let scene: CrewSceneAsset =
            read_json("assets/scenes/crew_axes.scene.json").expect("scene should load");
