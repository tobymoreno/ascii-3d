...
const SCENE_ASSET: &str = "assets/scenes/single_t_axes.scene.json";

#[derive(Debug, Clone, Deserialize)]
struct SingleTSceneAsset {
    name: String,
    version: u32,
...
fn load_scene_assets() -> io::Result<(
    ...
    let scene: SingleTSceneAsset = read_json(SCENE_ASSET)?;

    if scene.version != 1 {
        return Err(io::Error::other(format!(
            "unsupported single_t scene version {}",
            scene.version,
        )));
    }
    ...
    let axes_node = scene
        ...
        .ok_or_else(|| io::Error::other("single_t scene is missing cartesian_axes node"))?
        .clone();
    ...
    let word_node = scene
        ...
        .ok_or_else(|| io::Error::other("single_t scene is missing bezier_word node"))?
        .clone();

    if word_node.parent.as_deref() != Some(&axes_node.id) {
        return Err(io::Error::other(
            "single_t word node must be parented to the Cartesian axes node",
        ));
    }
    ...
    let word_metadata: WordMetadata = read_json("assets/words/single_t.metadata.json")?;
...
pub fn render(canvas: &mut Canvas, stroke_character: Option<char>) -> io::Result<()> {
    ...
    canvas.draw_text(
        Point2::new(2, 24),
        "Hierarchy: axes_root -> word_single_t -> glyph_T_0",
    );
    canvas.draw_text(
        Point2::new(2, 25),
        "Glyph T uses proportional 0..1 local space",
    );
...
#[cfg(test)]
mod tests {
    use super::{SingleTSceneAsset, read_json};

    #[test]
    fn single_t_scene_asset_loads() {
        let scene: SingleTSceneAsset =
            read_json("assets/scenes/single_t_axes.scene.json").expect("scene should load");
