...
const SCENE_ASSET: &str = "assets/scenes/single_e_axes.scene.json";

#[derive(Debug, Clone, Deserialize)]
struct SingleESceneAsset {
    name: String,
    version: u32,
...
fn load_scene_assets() -> io::Result<(
    ...
    let scene: SingleESceneAsset = read_json(SCENE_ASSET)?;

    if scene.version != 1 {
        return Err(io::Error::other(format!(
            "unsupported single_e scene version {}",
            scene.version,
        )));
    }
    ...
    let axes_node = scene
        ...
        .ok_or_else(|| io::Error::other("single_e scene is missing cartesian_axes node"))?
        .clone();
    ...
    let word_node = scene
        ...
        .ok_or_else(|| io::Error::other("single_e scene is missing bezier_word node"))?
        .clone();

    if word_node.parent.as_deref() != Some(&axes_node.id) {
        return Err(io::Error::other(
            "single_e word node must be parented to the Cartesian axes node",
        ));
    }
    ...
    let word_metadata: WordMetadata = read_json("assets/words/single_e.metadata.json")?;
...
pub fn render(canvas: &mut Canvas, stroke_character: Option<char>) -> io::Result<()> {
    ...
    canvas.draw_text(Point2::new(2, 1), "Scene: single_e word parent");
    canvas.draw_text(Point2::new(2, 2), &format!("Asset: {}", scene.name));
    canvas.draw_text(
        Point2::new(2, 24),
        "Hierarchy: axes_root -> word_single_e -> glyph_E_0",
    );
    canvas.draw_text(
        Point2::new(2, 25),
        "Glyph E uses proportional 0..1 local space",
    );
...
#[cfg(test)]
mod tests {
    use super::{SingleESceneAsset, read_json};

    #[test]
    fn single_e_scene_asset_loads() {
        let scene: SingleESceneAsset =
            read_json("assets/scenes/single_e_axes.scene.json").expect("scene should load");
